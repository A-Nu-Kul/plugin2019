 ...
 *
 * @package		Gandhipeacedonation/Classes/Gandhipeacedonation_Form
 * @version 	1.0.0
...
if ( ! class_exists( 'Gandhipeacedonation_Form' ) ) :

	/**
	 * Gandhipeacedonation_Form
	 *
	 * @since 1.0.0
	 */
	abstract class Gandhipeacedonation_Form {
		 ...
		 * @var   string
		 */
		protected $nonce_action = 'Gandhipeacedonation_Form';
		 ...
		 * @var   string
		 */
		protected $nonce_name = '_Gandhipeacedonation_Form_nonce';
		 ...
		 * @since 1.5.0
		 *
		 * @var   Gandhipeacedonation_Form_View_Interface
		 */
		protected $view;
		...
		protected function attach_hooks_and_filters() {
			gandhipeacedonation_get_deprecated()->doing_it_wrong(
				...
				sprintf(
					...
					__( 'None of the callbacks are required as of gandhipeacedonation 1.0; use `gandhipeacedonation_Public_Form_View` instead. Issue encountered in %s class.' ), get_class( $this )
				),
				'1.5.0'
			);

			add_action( 'Gandhipeacedonation_Form_before_fields', array( $this, 'render_error_notices' ) );
			add_action( 'Gandhipeacedonation_Form_before_fields', array( $this, 'add_hidden_fields' ) );
			add_action( 'Gandhipeacedonation_Form_field', array( $this, 'render_field' ), 10, 6 );
			add_filter( 'Gandhipeacedonation_Form_field_increment', array( $this, 'increment_index' ), 10, 2 );
		}
		 ...
		 * @since  1.5.0
		 *
		 * @param  Gandhipeacedonation_Form_View_Interface $form_view An object implementing the `Gandhipeacedonation_Form_View_Interface`.
		 * @return void
		 */
		public function set_form_view( Gandhipeacedonation_Form_View_Interface $form_view ) {
			$this->view = $form_view;
		}
		 ...
		 * @since  1.5.0
		 *
		 * @return Gandhipeacedonation_Form_View_Interface
		 */
		public function view() {
			if ( ! isset( $this->view ) ) {
				$this->view = new gandhipeacedonation_Public_Form_View( $this );
			}
		...
		public function get_hidden_fields() {
			 ...
			 * @param Gandhipeacedonation_Form $form   This instance of `Gandhipeacedonation_Form`.
			 */
			return apply_filters( 'Gandhipeacedonation_Form_hidden_fields', array(
				$this->nonce_name   => wp_create_nonce( $this->nonce_action ),
				'_wp_http_referer'  => wp_unslash( $_SERVER['REQUEST_URI'] ),
				'gandhipeacedonation_action' => $this->form_action,
			), $this );
		}
		...
		public function validate_nonce() {
			...
			if ( ! $validated ) {
				gandhipeacedonation_get_notices()->add_error( __( 'Unable to submit form. Please try again.', 'gandhipeacedonation' ) );

				if ( defined( 'gandhipeacedonation_DEBUG' ) && gandhipeacedonation_DEBUG ) {
					error_log(
						sprintf(
							/* translators: %1$s: nonce name; %2$s: nonce action; %3$s: submitted nonce */
							__( 'Nonce verification failed for nonce with name "%1$s" and action "%2$s". Submitted nonce is "%3$s".', 'gandhipeacedonation' ),
							$this->nonce_name,
							$this->nonce_action,
		...
		public function validate_honeypot() {
			...
			if ( ! isset( $submitted['Gandhipeacedonation_Form_id'] ) ) {
				return true;
			}

			$form_id = $submitted['Gandhipeacedonation_Form_id'];
		...
		public function check_required_fields( $fields, $submitted = array() ) {
			...
			foreach ( $this->get_required_fields( $fields ) as $key => $field ) {
				...
				$exists = apply_filters( 'gandhipeacedonation_required_field_exists', $exists, $key, $field, $submitted, $this );
			...
			$missing = apply_filters( 'Gandhipeacedonation_Form_missing_fields', $missing, $this, $fields, $submitted );

			if ( count( $missing ) ) {

				$missing_fields = implode( '</li><li>', $missing );

				gandhipeacedonation_get_notices()->add_error(
					sprintf( '<p>%s</p><ul class="error-list"><li>%s</li></ul>',
						__( 'There were problems with your form submission. The following required fields were not filled out:', 'gandhipeacedonation' ),
						$missing_fields
					)
				);
		...
		public function get_submitted_values() {
			if ( ! isset( $this->submitted ) ) {
				$this->submitted = apply_filters( 'Gandhipeacedonation_Form_submitted_values', $_POST, $this );
			}
		...
		protected function get_file_overrides( $file_key, $overrides = array() ) {
			...
			$defaults = array(
				'test_form' => false,
				'mimes'     => apply_filters( 'gandhipeacedonation_file_' . $file_key . '_allowed_mimes', $allowed_mimes ),
			);
		...
		public function get_template_name( $field ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'gandhipeacedonation_Public_Form_View::get_template_name()'
			);
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated. Implemented by Form View.
		 *
		...
		protected function is_valid_template( $template ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_Public_Form_View::is_valid_template()'
			);
		...
		protected function use_default_field_template( $field_type ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_Public_Form_View::use_default_field_template()'
			);
		...
		public function increment_index( $increment, $field ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_Public_Form_View::increment_index()'
			);
			 ...
			 * is called directly.
			 */
			remove_filter( 'Gandhipeacedonation_Form_field_increment', array( $this, 'increment_index' ), 10, 2 );
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated. Notices are rendered by the Form View now.
		 *
		 * @param  Gandhipeacedonation_Form $form Form object.
		...
		public function render_error_notices( $form ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_Public_Form_View::render_notices()'
			);
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated. Hidden fields are rendered by the Form View now.
		 *
		 * @param  Gandhipeacedonation_Form $form The form object.
		...
		public function add_hidden_fields( $form ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.5.0',
				'gandhipeacedonation_Public_Form_View::render_hidden_fields()'
			);
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated. Use `gandhipeacedonation_Public_Form_View` instead.
		 * @since  1.0.0 Added $from_template parameter for backwards compatibility purposes.
		 *               Anytime this hook is called from a template using the pre-gandhipeacedonation 1.5
		 *               approach of rendering fields (i.e. do_action( 'Gandhipeacedonation_Form_field' )),
		...
		public function render_field( $field, $key, $form, $index = 0, $namespace = null, $from_template = true ) {
			...
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_Public_Form_View::render_field()'
			);
