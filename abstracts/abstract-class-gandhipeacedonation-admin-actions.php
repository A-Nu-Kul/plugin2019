 ...
 *
 * @package   Gandhipeacedonation/Classes/Gandhipeacedonation_Admin_Actions
...
if ( ! class_exists( 'Gandhipeacedonation_Admin_Actions' ) ) :

	/**
	 * Gandhipeacedonation_Admin_Actions
	 *
	 * @since 1.0.0
	 */
	abstract class Gandhipeacedonation_Admin_Actions implements Gandhipeacedonation_Admin_Actions_Interface {
		...
		public function add_action_fields( $object_id, $action_id ) {
			...
			echo '<div class="gandhipeacedonation-action-fields" style="display: none;" data-type="' . esc_attr( $action_id ) . '">' . $fields . '</div>';
		}
		...
		public function do_action( $action, $object_id, $args = array() ) {
			...
			$action_hook = sprintf( 'gandhipeacedonation_%s_admin_action_%s', $this->get_type(), $action );
			 ...
			 *
			 * gandhipeacedonation_{type}_admin_action_{action key}
			 *
			 * @since 1.0.0
			 *
		...
		public function get_action_link( $action, $object_id ) {
			...
			return esc_url( add_query_arg( array(
				'gandhipeacedonation_admin_action' => $action,
		...
		protected function sanitize_action_args( $args ) {
			$defaults = array(
				'button_text' => __( 'Submit', 'gandhipeacedonation' ),
				'fields'      => false,
			);
