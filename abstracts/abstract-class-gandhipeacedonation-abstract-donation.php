 ...
 * Donation model.
 *
 * @package   Gandhipeacedonation/Classes/Gandhipeacedonation_Donation
...
if ( ! class_exists( 'Gandhipeacedonation_Abstract_Donation' ) ) :
	 ...
	 */
	abstract class Gandhipeacedonation_Abstract_Donation {
		...
		/**
		 * Gandhipeacedonation_Donation donation data for the donation plan this donation is part of.
		 *
		 * @since 1.4.5
		 *
		 * @var false|Gandhipeacedonation_Donation
		 */
		 ...
		 *
		 * @var Gandhipeacedonation_Campaign_Donations_DB
		 */
		 ...
		 *
		 * @var Gandhipeacedonation_Gateway_Interface
		 */
		protected $gateway;
		...
		/**
		 * The Gandhipeacedonation_Donor object of the person who donated.
		 *
		 * @var Gandhipeacedonation_Donor
		 */
		protected $donor;

		/**
		 * The Gandhipeacedonation_Donation_Log object for this donation.
		 *
		 * @since 1.0.0
		 *
		 * @var   Gandhipeacedonation_Donation_Log
		 */
		protected $log;

		/**
		 * The Gandhipeacedonation_Object_Fields object for this donation.
		 *
		 * @since 1.0.0
		 *
		 * @var   Gandhipeacedonation_Object_Fields
		 */
		protected $fields;
		...
		/**
		 * Return the Gandhipeacedonation_Object_Fields instance.
		 *
		 * @since  1.5.0
		 *
		 * @return Gandhipeacedonation_Object_Fields
		 */
		public function fields() {
			if ( ! isset( $this->fields ) ) {
				$this->fields = new Gandhipeacedonation_Object_Fields( gandhipeacedonation()->donation_fields(), $this );
			}
		...
		public function get_number() {
			 ...
			 */
			return apply_filters( 'Gandhipeacedonation_Donation_number', $this->donation_id );
		}
		 ...
		 * @since  1.0.0
		 *
		 * @return Gandhipeacedonation_Campaign_Donations_DB
		 */
		public function get_campaign_donations_db() {
			if ( ! isset( $this->campaign_donations_db ) ) {
				$this->campaign_donations_db = new Gandhipeacedonation_Campaign_Donations_DB();
			}
		...
		public function get_total_donation_amount( $sanitize = false ) {
			...
			if ( $sanitize ) {
				$amount = Gandhipeacedonation_Currency::get_instance()->sanitize_monetary_amount( (string) $amount );
			}
		...
		public function get_amount_formatted() {
			return gandhipeacedonation_format_money( $this->get_total_donation_amount() );
		}
			...
			foreach ( $this->get_campaign_donations() as $campaign ) {
				...
				if ( ! isset( $links[ $campaign->campaign_id ] ) ) {
					...
					$links[ $campaign->campaign_id ] = sprintf( '<a href="%s" aria-label="%s">%s</a>',
						...
						sprintf( '%s %s', _x( 'Go to', 'go to campaign', 'gandhipeacedonation' ), get_the_title( $campaign->campaign_id ) ),
		 ...
		 * @uses   wp_list_pluck
		 * @uses   Gandhipeacedonation_Donation::get_campaign_donations
		 *
		 * @since  1.0.0
		 *
		 ...
		 *
		 * @uses   Gandhipeacedonation_Donation::get_campaign_categories_donated_to
		 *
		 * @since  1.0.0
		 *
		 * @return string
		...
		protected function get_donation_summary_line( $campaign_donation ) {
			$line_item = sprintf( '%s: %s%s',
				...
				gandhipeacedonation_format_money( $campaign_donation->amount ),
				PHP_EOL
			);
			 ...
			 *
			 * @since 1.0.0
			 * @since 1.5.0 Filter was previously defined in Gandhipeacedonation_Email::get_donation_summary()
			 *              and included an array of shortcode arguments and a Gandhipeacedonation_Email object.
			 *
			 * @param string $line_item         The line of text.
			 * @param object $campaign_donation A campaign donation object.
			 */
			return apply_filters( 'Gandhipeacedonation_Donation_summary_line_item_email', $line_item, $campaign_donation );
		}
		...
		public function get_gateway_label() {
			 ...
			 * @param Gandhipeacedonation_Donation $donation This `Gandhipeacedonation_Donation` instance.
			 */
			return apply_filters( 'Gandhipeacedonation_Donation_gateway_label', $label, $this );
		}
		 ...
		 * @since  1.0.0
		 *
		 * @return false|Gandhipeacedonation_Gateway
		 */
		public function get_gateway_object() {
			$class = gandhipeacedonation_get_helper( 'gateways' )->get_gateway( $this->get_gateway() );
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated $label argument. Use Gandhipeacedonation_Donation::get_status_label() instead.
		 *
		...
		public function get_status( $label = false ) {
			...
			if ( $label ) {
				gandhipeacedonation_get_deprecated()->deprecated_argument(
					__METHOD__,
					'1.5.0',
					__( 'The $label argument is deprecated. Use Gandhipeacedonation_Donation::get_status_label() instead.', 'gandhipeacedonation' )
				);
		...
		public function get_status_label( $status = '' ) {
			$statuses = gandhipeacedonation_get_valid_donation_statuses();
		...
		public function has_status( $status ) {
			 ...
			 * @param Gandhipeacedonation_Donation $donation   This instance of `Gandhipeacedonation_Donation`.
			 * @param string|array        $status     The status we are checking for.
			 */
			return apply_filters( 'Gandhipeacedonation_Donation_has_status', $has_status, $this, $status );
		}
		 ...
		 * @since  1.0.0
		 *
		 * @return Gandhipeacedonation_Donor
		 */
		public function get_donor() {
			if ( ! isset( $this->donor ) ) {
				$this->donor = new Gandhipeacedonation_Donor( $this->get_donor_id(), $this->donation_id );
			}
		...
		public function get_test_mode( $text = true ) {
			...
			if ( $text ) {
				return $test_mode ? __( 'Yes', 'gandhipeacedonation' ) : __( 'No', 'gandhipeacedonation' );
			}
		...
		public function get_contact_consent() {
			...
			if ( ! $this->get_donor()->contact_consent ) {
				return $consent ? __( 'Withdrawn', 'gandhipeacedonation' ) : __( 'Not Given', 'gandhipeacedonation' );
			}

			return $consent ? __( 'Given', 'gandhipeacedonation' ) : __( 'Given Later', 'gandhipeacedonation' );
		}
		...
		public function get_donation_meta() {
			$fields = gandhipeacedonation()->donation_fields()->get_meta_fields();
			 ...
			 * @param Gandhipeacedonation_Donation $donation This instance of `Gandhipeacedonation_Donation`.
			 */
			return apply_filters( 'Gandhipeacedonation_Donation_admin_meta', $meta, $this );
		}
		...
		public function is_from_current_user() {
			...
			if ( gandhipeacedonation_get_session()->has_donation_key( $this->get_donation_key() ) ) {
				return true;
			}
		 ...
		 * @since  1.5.4
		 *
		 * @return Gandhipeacedonation_Donation_Log
		 */
		public function log() {
			if ( ! isset( $this->log ) ) {
				$this->log = new Gandhipeacedonation_Donation_Log( $this->ID );
			}
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated $donation_id arg. Method is now expected to be called in an object context.
		...
		public function update_status( $new_status ) {
			...
			if ( false === gandhipeacedonation_is_valid_donation_status( $new_status ) ) {
			...
			$message = sprintf(
				...
				__( 'Donation status updated from %1$s to %2$s.', 'gandhipeacedonation' ),
			...
			/**
			 * Handle specific gandhipeacedonation status transitions.
			 *
			 * @since 1.0.0
			 *
			 * @param Gandhipeacedonation_Donation $donation This instance of `Gandhipeacedonation_Donation`.
			 */
			do_action( 'Gandhipeacedonation_Donation_status_from_' . $old_status . '_to_' . $new_status, $this );

			/**
			 * Handle specific gandhipeacedonation status change.
			 *
			 * @since 1.0.0
			 *
			 * @param Gandhipeacedonation_Donation $donation   This instance of `Gandhipeacedonation_Donation`.
			 ...
			 */
			do_action( 'Gandhipeacedonation_Donation_status_' . $new_status, $this, $old_status );

			/**
			 * Handle all gandhipeacedonation status changes.
			 *
			 * @since 1.0.0
			 *
			 * @param Gandhipeacedonation_Donation $donation   This instance of `Gandhipeacedonation_Donation`.
			 ...
			 * @param string              $old_status The previous donation status.
			 */
			do_action( 'Gandhipeacedonation_Donation_status_changed', $this, $new_status, $old_status );
		 ...
		 *
		 * @since  1.0.0
		 * @since  1.0.0 First parameter changed from $donation_id to $message, as function is now expected
		 *               to be used in an object context. i.e.: $donation->update_donation_log( 'my message' )
		 * @since  1.0.0 Removed second parameter ($deprecated_message), which was only used for backwards
		 *               compatibility. We now use func_get_arg() instead to get the second passed argument.
		 * @since  1.0.0 Now uses Gandhipeacedonation_Donation_Log::add() and returns a result.
		 *
		 ...
		 * @since  1.0.0
		 *
		 * @return false|Gandhipeacedonation_Donation
		 */
		public function get_donation_plan() {
			if ( ! isset( $this->donation_plan ) ) {
				if ( $this->donation_data->post_parent > 0 ) {
					$this->donation_plan = gandhipeacedonation_get_donation( $this->donation_data->post_parent );
				} else {
		...
		public function set_gateway_transaction_id( $value ) {
			...
			$value = gandhipeacedonation_sanitize_donation_meta( $value, $key );
		 ...
		 * @since  1.5.0
		 *
		 * @param  Gandhipeacedonation_Donation_Field $field Instance of `Gandhipeacedonation_Donation_Field`.
		 * @return array
		 */
		protected function parse_donation_meta_field( Gandhipeacedonation_Donation_Field $field ) {
			$value = gandhipeacedonation_get_sanitized_donation_field_value( $this->get( $field->field ), $field->field );
		 ...
		 * @deprecated 1.0.0
		 *
		 * @see    gandhipeacedonation_get_valid_donation_statuses
		 *
		 * @since  1.0.0
		...
		public function get_valid_donation_statuses() {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_get_valid_donation_statuses'
			);

			return gandhipeacedonation_get_valid_donation_statuses();
		}
		 ...
		 * @deprecated 1.0.0
		 *
		 * @see    gandhipeacedonation_is_valid_donation_status
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated.
		 *
		...
		public function is_valid_donation_status( $status ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_is_valid_donation_status'
			);

			return gandhipeacedonation_is_valid_donation_status( $status );
		}

		/**
		 * Returns the donation statuses that signify a donation was complete.
		 *
		 * By default, this is just 'gandhipeacedonation-completed'. However, 'gandhipeacedonation-preapproval'
		 * is also counted.
		 *
		 * @deprecated 1.0.0
		 *
		 * @see    gandhipeacedonation_get_approval_statuses
		 *
		 * @since  1.0.0
		 *
		 * @return string[]
		...
		public function get_approval_statuses() {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_get_approval_statuses'
			);

			return gandhipeacedonation_get_approval_statuses();
		}
		 ...
		 * @deprecated 1.0.0
		 *
		 * @see    gandhipeacedonation_is_approved_status
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated.
		 *
		...
		public function is_approved_status( $status ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_is_approved_status'
			);

			return gandhipeacedonation_is_approved_status( $status );
		}
		 ...
		 * @deprecated 1.0.0
		 *
		 * @see    gandhipeacedonation_sanitize_donation_meta
		 *
		 * @since  1.0.0
		 * @since  1.0.0
		 *
		...
		public function sanitize_meta( $value, $key ) {
			gandhipeacedonation_get_deprecated()->deprecated_function(
				__METHOD__,
				'1.0.0',
				'gandhipeacedonation_sanitize_donation_meta()'
			);

			return gandhipeacedonation_sanitize_donation_meta( $value, $key );
		}
		 ...
		 * @deprecated 1.0.0
		 *
		 * @see    gandhipeacedonation_flush_campaigns_donation_cache
		 *
		 * @since  1.0.0
		 * @since  1.0.0 Deprecated.
		 *
		...
		public function flush_campaigns_donation_cache( $donation_id ) {
			...
			return gandhipeacedonation_flush_campaigns_donation_cache( $donation_id );
		}
	}
