 ...
 *
 * @package     Gandhipeacedonation/Classes/Gandhipeacedonation_Export
 * @version     1.0.0
...
if ( ! class_exists( 'Gandhipeacedonation_Export' ) ) :

	/**
	 * Gandhipeacedonation_Export
	 *
	 * @abstract
	 * @since   1.0.0
	 */
	abstract class Gandhipeacedonation_Export {
		...
		public function can_export() {
			return (bool) apply_filters( 'Gandhipeacedonation_Export_capability', current_user_can( 'export_gandhipeacedonation_reports' ), $this );
		}
		...
		protected function map_data( $data ) {
			...
			foreach ( $this->columns as $key => $label ) {
				...
				$value = apply_filters( 'Gandhipeacedonation_Export_data_key_value', $value, $key, $data );
				$row[] = $value;
			}
		...
		protected function print_headers() {
			...
			if ( ! gandhipeacedonation_is_func_disabled( 'set_time_limit' ) && ! ini_get( 'safe_mode' ) ) {
				set_time_limit( 0 );
			}
			...
			header( 'Content-Disposition: attachment; filename=gandhipeacedonation-export-' . $export . '-' . date( 'm-d-Y' ) . '.csv' );
			header( 'Expires: 0' );
		}
