<!-- Begin Gandhipeace Donation by http://anukuls.sgedu.site/plugin -->
<?php
$url = isset($pd_options['sandbox']) ? 'https://www.sandbox.paypal.com/cgi-bin/webscr' : 'https://www.paypal.com/cgi-bin/webscr';
?>

<form action="<?php echo apply_filters('gandhipeace-donation_url', $url); ?>" method="post"<?php
if (isset($pd_options['new_tab'])) {
        echo ' target="_blank"';
}
?>>
    <div class="gandhipeace-donation">
        <input type="hidden" name="cmd" value="_donations" />
        <input type="hidden" name="bn" value="AnukulJcub_SP" />
        <input type="hidden" name="business" value="<?php echo apply_filters('gandhipeace-donation_merchant_email', $pd_options['paypal_account']); ?>" />
<?php
        # Build the button
        $paypal_btn = '';
        $indent = str_repeat(" ", 8);
        ...
        if ($purpose)
            $paypal_btn .=  apply_filters('gandhipeace-donation_purpose_html', $indent.'<input type="hidden" name="item_name" value="' .esc_attr($purpose). '" />'.PHP_EOL);  // Purpose
        if ($reference)
        ...
        if ($amount){
            ...
            $paypal_btn .=  $indent.'<input type="hidden" name="amount" value="' . apply_filters( 'gandhipeace-donation_amount', $amount ) . '" />'.PHP_EOL;
        }

        if (!empty($validate_ipn)){
            $notify_url = site_url() . '/?gpd_paypal_ipn=process';
        ...
        $paypal_btn .=  $indent.'<input type="image" style="cursor: pointer;" src="button.jpg" .esc_url($button_url). '" name="submit" alt="Donate Now" />'.PHP_EOL;
